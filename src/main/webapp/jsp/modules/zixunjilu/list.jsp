...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">咨询记录管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>咨询记录列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">咨询记录列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<label>咨询日期</label>
									<input id="zixunriqi_start" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
									至
									<input id="zixunriqi_end" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th>景点图片</th>
											<th onclick="sort('zixunriqi')">咨询日期<i id="zixunriqiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#zixunriqi_start').val() != null && $('#zixunriqi_start').val() != ''){
				searchForm.zixunriqi_start = $('#zixunriqi_start').val();
			}
			if($('#zixunriqi_end').val() != null && $('#zixunriqi_end').val() != ''){
				searchForm.zixunriqi_end = $('#zixunriqi_end').val();
			}
        ...
        function getDataList() {
			http("zixunjilu/page","GET",{
				...
				zixunriqi_start : searchForm.zixunriqi_start,
				zixunriqi_end : searchForm.zixunriqi_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var zixunriqiCell  = document.createElement('td');

						  zixunriqiCell.innerHTML = item.zixunriqi;


			row.appendChild(zixunriqiCell);
			var zixunneirongCell  = document.createElement('td');
		...
		function sh(){
			...
			httpJson("zixunjilu/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("zixunjilu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"zixunjilu");
