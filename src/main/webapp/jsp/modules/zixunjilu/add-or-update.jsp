...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑咨询记录</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>咨询记录管理</span></li>
								<li class="breadcrumb-item active"><span>编辑咨询记录</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">咨询记录信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>咨询日期</label>
										<div id="zixunriqiDate" class="input-append date form_datetime">
											<input id="zixunriqi-input" name="zixunriqi" size="20" type="text" readonly>
									...
									<div class="form-group col-md-6">
										...
										<input name="shouji" id="shouji" name="shouji" class="form-control" placeholder="手机">
											...
											<div class="form-group  col-md-12" >
												<label>咨询内容</label>
												<div><div class="upload"><input id="zixunneirongupload" name="file" type="file">插入图片</div>
												<script id="zixunneirongEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var zixunneirongUe = UE.getEditor('zixunneirongEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="zixunneirong" id="zixunneirong-input">
												</div>
											</div>	
	...
	<script>
		...
		function upload(){
			...
			$('#zixunneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('zixunneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
               //获取表单数据前取消禁用
                $("#zixunriqi-input").attr('disabled',false);
				...
				if(crossrefid && crossuserid) {
					...
					http("zixunjilu/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("zixunjilu/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.zixunneirong != null && ruleForm.zixunneirong != 'null' && ruleForm.zixunneirong != ''){
				var zixunneirongMes = '' + ruleForm.zixunneirong;
				var zixunneirongUeditor = UE.getEditor('zixunneirongEditor');
				zixunneirongUeditor.ready(function() {
					zixunneirongUeditor.setContent(zixunneirongMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('zixunneirongEditor').hasContents()){
				$('#zixunneirong-input').attr('value',UE.getEditor('zixunneirongEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("zixunjilu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#zixunriqi-input").val(getCurDateTime());
			$("#zixunriqiDate").datetimepicker({ 
				autoclose: true,
		...
		function dataBind(){
			...
			$("#zixunriqi-input").val(ruleForm.zixunriqi);
			$("#zixunneirong").val(ruleForm.zixunneirong);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.zixunriqi != null && res.data.zixunriqi != ''){

						$("#zixunriqi-input").val(res.data.zixunriqi);
						$('#zixunriqiFileName').val(res.data.zixunriqi)
					}
					if(res.data != null && res.data != '' && res.data.zixunneirong != null && res.data.zixunneirong != ''){

						$("#zixunneirong").val(res.data.zixunneirong);
                        var zixunneirongMes = '' + res.data.zixunneirong;
                        var zixunneirongUeditor = UE.getEditor('zixunneirongEditor');
                        zixunneirongUeditor.ready(function() {
                            zixunneirongUeditor.setContent(zixunneirongMes);
                        });
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#jingdianmingcheng").attr('readonly','readonly');
                $("#jingdiantupian").attr('readonly','readonly');
                $("#zixunriqi-input").attr('disabled',true);
                $("#yonghuming").attr('readonly','readonly');
                $("#shouji").attr('readonly','readonly');
            }
		}
