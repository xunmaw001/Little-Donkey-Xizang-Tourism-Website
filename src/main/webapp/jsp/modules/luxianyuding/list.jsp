...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">路线预定管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>路线预定管理</span></li>
								<li class="breadcrumb-item active"><span>路线预定列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">路线预定列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<label>预定时间</label>
									<input id="yudingshijian_start" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
									至
									<input id="yudingshijian_end" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th>路线图片</th>
											<th onclick="sort('yudingshijian')">预定时间<i id="yudingshijianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('yudingneirong')">预定内容<i id="yudingneirongIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#yudingshijian_start').val() != null && $('#yudingshijian_start').val() != ''){
				searchForm.yudingshijian_start = $('#yudingshijian_start').val();
			}
			if($('#yudingshijian_end').val() != null && $('#yudingshijian_end').val() != ''){
				searchForm.yudingshijian_end = $('#yudingshijian_end').val();
			}
        ...
        function getDataList() {
			http("luxianyuding/page","GET",{
				...
				yudingshijian_start : searchForm.yudingshijian_start,
				yudingshijian_end : searchForm.yudingshijian_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var yudingneirongCell  = document.createElement('td');

						 yudingneirongCell.innerHTML = item.yudingneirong;

			row.appendChild(yudingneirongCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("luxianyuding/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("luxianyuding/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("luxianyuding/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"luxianyuding");
