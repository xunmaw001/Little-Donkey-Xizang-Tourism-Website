...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">旅游路线管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>旅游路线列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">旅游路线列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										路线名称
									</label>
									<input type="text" id="luxianmingchengSearch" class="form-control form-control-sm" placeholder="请输入路线名称" aria-controls="tableId">
									<label>
										景点名称
									</label>
									...
									<input type="text" id="qidianSearch" class="form-control form-control-sm" placeholder="请输入起点" aria-controls="tableId">
									<label>
										出行方式
									</label>
									<input type="text" id="chuxingfangshiSearch" class="form-control form-control-sm" placeholder="请输入出行方式" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('luxianmingcheng')">路线名称<i id="luxianmingchengIcon" class="fa fa-sort"></i></th>
											...
											<th>路线图片</th>
											<th onclick="sort('qidian')">起点<i id="qidianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('tujingluduan')">途径路段<i id="tujingluduanIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('zhongdian')">终点<i id="zhongdianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('chuxingfangshi')">出行方式<i id="chuxingfangshiIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#luxianmingchengSearch').val() != null && $('#luxianmingchengSearch').val() != ''){
				searchForm.luxianmingcheng ="%" +  $('#luxianmingchengSearch').val() + "%";
			}
			...
			if($('#qidianSearch').val() != null && $('#qidianSearch').val() != ''){
				searchForm.qidian ="%" +  $('#qidianSearch').val() + "%";
			}
			
			if($('#chuxingfangshiSearch').val() != null && $('#chuxingfangshiSearch').val() != ''){
				searchForm.chuxingfangshi ="%" +  $('#chuxingfangshiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("lvyouluxian/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				luxianmingcheng : searchForm.luxianmingcheng,	
				jingdianmingcheng : searchForm.jingdianmingcheng,	
				qidian : searchForm.qidian,	
				chuxingfangshi : searchForm.chuxingfangshi,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var luxianmingchengCell  = document.createElement('td');

						 luxianmingchengCell.innerHTML = item.luxianmingcheng;

			row.appendChild(luxianmingchengCell);
			...
			var luxiantupianCell  = document.createElement('td');

				                var luxiantupianImg = document.createElement('img');
			luxiantupianImg.setAttribute('src',baseUrl+item.luxiantupian);
			luxiantupianImg.setAttribute('height',100);
			luxiantupianImg.setAttribute('width',100);
			luxiantupianCell.appendChild(luxiantupianImg);


			row.appendChild(luxiantupianCell);
			var qidianCell  = document.createElement('td');

						 qidianCell.innerHTML = item.qidian;

			row.appendChild(qidianCell);
			var tujingluduanCell  = document.createElement('td');

						 tujingluduanCell.innerHTML = item.tujingluduan;

			row.appendChild(tujingluduanCell);
			var zhongdianCell  = document.createElement('td');

						 zhongdianCell.innerHTML = item.zhongdian;

			row.appendChild(zhongdianCell);
			var chuxingfangshiCell  = document.createElement('td');

						 chuxingfangshiCell.innerHTML = item.chuxingfangshi;

			row.appendChild(chuxingfangshiCell);
			var luxianxiangqingCell  = document.createElement('td');
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "路线预定"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'luxianyuding','','','')");
			if(crossBtnControl('路线预定') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("lvyouluxian/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusslvyouluxian/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"lvyouluxian");
