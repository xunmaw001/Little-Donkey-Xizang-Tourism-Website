...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">景点信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>景点信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">景点信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="jingdianxingji" id="jingdianxingjiSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
									...
									<input type="text" id="jingdiandizhiSearch" class="form-control form-control-sm" placeholder="请输入景点地址" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('jingdianxingji')">景点星级<i id="jingdianxingjiIcon" class="fa fa-sort"></i></th>
											<th>景点图片</th>
											<th onclick="sort('menpiaojiage')">门票价格<i id="menpiaojiageIcon" class="fa fa-sort"></i></th>
											...
											<th>宣传视频</th>
											<th onclick="sort('zixunrexian')">咨询热线<i id="zixunrexianIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			...
			var jingdianxingjiIndex = document.getElementById("jingdianxingjiSelect").selectedIndex;
			var jingdianxingjiOptions = document.getElementById("jingdianxingjiSelect").options;
			var jingdianxingjiSelectedOption = jingdianxingjiOptions[jingdianxingjiIndex]
			if(jingdianxingjiSelectedOption.text != null && jingdianxingjiSelectedOption.text != ''){
				searchForm.jingdianxingji = jingdianxingjiSelectedOption.text;
			}
			if($('#jingdiandizhiSearch').val() != null && $('#jingdiandizhiSearch').val() != ''){
				searchForm.jingdiandizhi ="%" +  $('#jingdiandizhiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("jingdianxinxi/page","GET",{
				...
				jingdianxingji : searchForm.jingdianxingji,	
				jingdiandizhi : searchForm.jingdiandizhi,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var jingdianxingjiCell  = document.createElement('td');

						 jingdianxingjiCell.innerHTML = item.jingdianxingji;

			row.appendChild(jingdianxingjiCell);
			...
			var zixunrexianCell  = document.createElement('td');

						 zixunrexianCell.innerHTML = item.zixunrexian;

			row.appendChild(zixunrexianCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "门票购买"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'menpiaogoumai','','','')");
			if(crossBtnControl('门票购买') == true){
				btnGroup.appendChild(crossBtn0);
			}   
			var crossBtn1 = document.createElement('button');
            		crossBtn1.innerHTML = "套餐咨询"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'zixunjilu','','','')");
			if(crossBtnControl('套餐咨询') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jingdianxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussjingdianxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"jingdianxinxi");
